...
function generateProceduralFace(seed: number): HTMLCanvasElement {
  ...
  // Clear background with soft slate tinted frame
...
export const ApkLab: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      links.forEach(link => {
        ...
        const baseAlpha = isHighlighted ? 0.95 : isDimmed ? 0.15 : 0.45;
        const colorString = isHighlighted 
          ? `rgba(234, 88, 12, ${baseAlpha})` 
          : `rgba(203, 213, 225, ${baseAlpha})`;
      ...
      for (let r = minRowVisible; r <= maxRowVisible; r++) {
        for (let c = minColVisible; c <= maxColVisible; c++) {
          ...
          // Render Target Bounding Box overlays
          if (node.isTargeted) {
      ...
      if (isGlitching.current && Math.random() < 0.45) {
        ...
        ctx.fillStyle = Math.random() > 0.5 ? 'rgba(234, 88, 12, 0.08)' : 'rgba(249, 115, 22, 0.08)';
  ...
  return (
    <div className="w-full h-full flex flex-col bg-white rounded-2xl border border-slate-200 overflow-hidden text-left font-sans shadow-sm relative select-none">
      
      {/* Upper-left HUD system diagnostics */}
      <div className="absolute top-5 left-5 z-20 pointer-events-none font-mono flex flex-col gap-1.5 bg-white/90 border border-slate-200 p-3.5 rounded-xl backdrop-blur-md shadow-sm">
        <div className="flex items-center gap-2 mb-1 pb-1.5 border-b border-slate-100">
          <Activity className="text-orange-600 animate-pulse" size={13} />
          <span className="text-[10px] uppercase font-black tracking-wider text-slate-800">SYNDICATE TRACKING HUD</span>
        </div>
        
        <div className="flex gap-4 text-[9px]">
          <span className="text-slate-500">SIMULATED MEMBERS:</span>
          <span className="text-slate-900 font-bold font-mono">2,000 / 2,000</span>
        </div>
        <div className="flex gap-4 text-[9px]">
          <span className="text-slate-500">INTERACTIVE LINK CHANNELS:</span>
          <span className="text-orange-600 font-bold font-mono">{links.length} Active</span>
        </div>
        <div className="flex gap-4 text-[9px]">
          <span className="text-slate-500">CAMERA MAGNIFICATION:</span>
          <span className="text-slate-900 font-bold font-mono">{Math.round(cameraDisplayState.z * 100)}%</span>
        </div>
        <div className="flex gap-4 text-[9px]">
          <span className="text-slate-500">SYSTEM MODE:</span>
          <span className={`font-black uppercase flex items-center gap-1 ${isAutoZoom ? 'text-orange-600 animate-pulse' : 'text-slate-500'}`}>
      ...
      <div className="absolute top-5 right-5 z-20 flex flex-wrap gap-2">
        <button
          ...
          className={`px-3.5 py-2 rounded-xl text-[9px] font-mono font-black uppercase tracking-wider transition-all cursor-pointer border flex items-center gap-1.5 shadow-sm ${
            isAutoZoom 
        ...
        <button
          ...
          className="px-3.5 py-2 bg-white text-slate-700 border border-slate-200 hover:text-slate-900 hover:bg-slate-50 rounded-xl text-[9px] font-mono font-black uppercase tracking-wider transition-all cursor-pointer flex items-center gap-1.5 shadow-sm"
        ...
        {packets.length > 0 && (
          <button
            ...
            className="px-3.5 py-2 bg-orange-50 text-orange-600 border border-orange-200 hover:bg-orange-100 rounded-xl text-[9px] font-mono font-black uppercase tracking-wider transition-all cursor-pointer flex items-center gap-1.5 shadow-sm"
          >
            <Zap size={11} />
      ...
      <div className="flex-1 w-full h-full relative overflow-hidden min-h-0">
        ...
        <AnimatePresence>
          {selectedNodeData && (
            <motion.div
              ...
              className="absolute z-30 bg-white/95 border border-orange-200 text-slate-900 rounded-xl p-4 w-64 shadow-xl backdrop-blur-xl font-mono"
            >
              {/* Tooltip Header */}
              <div className="flex items-center justify-between border-b border-slate-100 pb-2 mb-3">
                <div className="flex items-center gap-1.5">
                  <Target size={12} className="text-orange-600 animate-pulse" />
                  <span className="text-[10px] uppercase font-black tracking-wider text-orange-600">TARGET SELECTED</span>
                </div>
                <button 
              ...
              <div className="flex gap-3 mb-3 bg-slate-50 p-2.5 rounded-lg border border-slate-200">
                <div className="w-12 h-12 border border-slate-200 rounded overflow-hidden shrink-0 bg-slate-900 flex items-center justify-center">
                  <canvas
                    ref={(el) => {
                      if (el) {
                ...
                <div className="overflow-hidden">
                  <h4 className="text-xs font-black truncate text-slate-900 leading-tight uppercase">{selectedNodeData.node.name}</h4>
                  <span className="text-[8px] text-slate-500 uppercase block mt-0.5">IP: {selectedNodeData.node.nodeIp}</span>
                  <span className="text-[8px] text-orange-600 uppercase block mt-0.5">{selectedNodeData.node.country} Registry</span>
                </div>
              </div>
              ...
              <div className="space-y-2 text-[9px] mb-4">
                <div className="flex justify-between border-b border-slate-100 pb-1">
                  <span className="text-slate-500">NODE VALUE ID:</span>
                  <span className="text-slate-900 font-bold">0x{selectedNodeData.node.id.toUpperCase().replace('_', '')}</span>
                </div>
                <div className="flex justify-between border-b border-slate-100 pb-1">
                  <span className="text-slate-500">SYNTHETIC RISK:</span>
                  <span className={`font-black ${selectedNodeData.node.riskScore > 85 ? 'text-orange-600 animate-pulse' : 'text-amber-600'}`}>
                    {selectedNodeData.node.riskScore}% CRITICAL
                  </span>
                </div>
                <div className="flex justify-between border-b border-slate-100 pb-1">
                  <span className="text-slate-500">DIVERTED CAPITALS:</span>
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-orange-600 font-bold uppercase tracking-wider">{selectedNodeData.node.status}</span>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-1.5 shrink-0">
                <button
                  ...
                  className="w-full bg-orange-600 hover:bg-orange-700 text-white text-[9px] font-black uppercase tracking-wider py-2 rounded-lg transition-all cursor-pointer flex items-center justify-center gap-1 shadow-sm"
                >
                  <Zap size={10} />
                ...
                <button
                  ...
                  className="w-full bg-slate-100 hover:bg-slate-200 border border-slate-200 text-slate-800 text-[9px] font-black uppercase tracking-wider py-2 rounded-lg transition-all cursor-pointer flex items-center justify-center gap-1"
                >
        ...
        <div className="absolute bottom-5 right-5 z-20 max-w-[210px] bg-white/90 border border-slate-200 p-3 rounded-xl backdrop-blur-md font-mono pointer-events-auto shadow-sm">
          <div className="flex items-center gap-1.5 text-[10px] font-black text-orange-600 uppercase border-b border-slate-100 pb-1 mb-1.5">
            <Compass size={11} />
            <span>Interactive Guide</span>
          </div>
          <p className="text-[8px] text-slate-600 leading-relaxed font-mono">
            • <b className="text-slate-900">Mouse Scroll:</b> Fly-by scale zoom<br />
            • <b className="text-slate-900">Click & Drag:</b> Pan matrix coordinates<br />
            • <b className="text-slate-900">Hover Nodes:</b> Isolate identity link paths<br />
            • <b className="text-slate-900">Click Node:</b> Open twin dossier / route packet burst
          </p>
        </div>

        {/* Minimalist Radar minimap bottom-left */}
        <div className="absolute bottom-5 left-5 z-20 w-24 h-24 bg-white/90 border border-slate-200 rounded-xl backdrop-blur-md overflow-hidden p-1 pointer-events-none hidden sm:block shadow-sm">
          <div className="text-[7.5px] font-mono font-black tracking-wider text-slate-400 text-center mb-1 uppercase">RADAR HUD</div>
          <div className="w-full h-16 border border-slate-200 relative bg-slate-50">
            ...
            <div 
              ...
              className="absolute border border-orange-500 bg-orange-500/10 transition-all duration-75"
            />
            ...
            {nodes.filter(n => n.isTargeted).slice(0, 40).map((n, i) => (
              <div 
                ...
                className="absolute w-0.5 h-0.5 bg-orange-500/60 rounded-full"
              />
            ))}
            {/* Sweeping scanline */}
            <div className="absolute inset-x-0 h-[1.5px] bg-orange-500/30 animate-scanline pointer-events-none" />
          </div>
        </div>
      </div>
    </div>
