...
const NUCLEOTIDE_COLORS = [
  new THREE.Color('#EA580C'), // Vibrant Argus Orange
  new THREE.Color('#0284C7'), // Deep Sky Blue
  new THREE.Color('#059669'), // Vivid Emerald
  new THREE.Color('#D97706'), // Amber Gold
];

const NUCLEOTIDE_HEX_COLORS = [
  '#EA580C', // Vibrant Argus Orange
...
export const ThreatOracle: React.FC<ThreatOracleProps> = () => {
  ...
  return (
    <div 
      ref={containerRef}
      className="w-full h-full min-h-[350px] relative bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col justify-between font-mono text-left select-none group"
    >
      {/* Background Matrix Scanning HUD Elements */}
      <div className="absolute inset-0 pointer-events-none z-10">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(0,0,0,0.04)_1px,transparent_1px)] bg-[size:16px_16px] opacity-70" />
        ...
        <div className="absolute top-4 left-6 flex flex-col gap-0.5">
          ...
          <span className="text-[7.5px] font-bold text-slate-400 uppercase tracking-widest pl-3">
            NEURAL HEURISTIC THREAT VISUALIZATION
          </span>
        </div>
        <div className="absolute top-4 right-6 text-right">
          <span className="text-[8.5px] font-bold text-orange-600 uppercase tracking-wider animate-pulse">
      ...
      <div className="absolute left-6 top-1/2 -translate-y-1/2 flex flex-col gap-5 z-20 pointer-events-none select-none text-[8px] tracking-[0.2em] text-slate-500">
        <div>
          <span className="block opacity-40 uppercase text-[6px]">ENGINE_CLASS</span>
          <span className="text-slate-800 font-bold">DEEP_GNN_HEURISTIC</span>
        </div>
        <div>
          <span className="block opacity-40 uppercase text-[6px]">VISUALIZATION_FLOW</span>
          <span className={`font-bold transition-all ${currentPhase === 'IRIS' ? 'text-orange-600 font-black' : 'text-slate-700'}`}>
            {currentPhase}
          </span>
        </div>
        <div>
          <span className="block opacity-40 uppercase text-[6px]">FIBER_STREAM_COUNT</span>
          <span className="text-slate-800 font-bold">140 STREAMS</span>
        </div>
        <div>
          <span className="block opacity-40 uppercase text-[6px]">GRID_SCALE</span>
          <span className="text-slate-800 font-bold">3D_SPATIAL_1.0x</span>
        </div>
      </div>

      {/* Floating Right Status Rail Overlay */}
      <div className="absolute right-6 top-1/2 -translate-y-1/2 flex flex-col gap-5 z-20 text-right text-[8px] tracking-[0.18em] text-slate-500">
        <div className="pointer-events-none">
          <span className="block opacity-40 uppercase text-[6px]">PUPIL_DILATION_RATIO</span>
          <span className="text-slate-800 font-bold font-mono">
            {currentPhase === 'IRIS' ? `${pupilDilation}%` : 'CALIBRATING'}
          </span>
        </div>
        <div className="pointer-events-none">
          <span className="block opacity-40 uppercase text-[6px]">HORIZONTAL_GAZE</span>
          <span className="text-slate-800 font-bold">
            {parallaxAngle.x > 0 ? '+' : ''}{parallaxAngle.x.toFixed(1)}°
          </span>
        </div>
        <div className="pointer-events-none">
          <span className="block opacity-40 uppercase text-[6px]">VERTICAL_GAZE</span>
          <span className="text-slate-800 font-bold">
            {parallaxAngle.y > 0 ? '+' : ''}{parallaxAngle.y.toFixed(1)}°
          </span>
        </div>
        <div className="pointer-events-none">
          <span className="block opacity-40 uppercase text-[6px]">SIMULATION_TIME</span>
          <span className="text-slate-800 font-bold font-mono">{simulationTime.toFixed(2)}S</span>
        </div>
        ...
        <button 
          ...
          className="mt-2 bg-slate-100 hover:bg-slate-200 active:bg-slate-300 border border-slate-200 px-2.5 py-1.5 rounded-lg text-slate-700 text-[7.5px] font-bold uppercase tracking-widest flex items-center gap-1.5 self-end transition-all cursor-pointer pointer-events-auto shadow-sm"
        >
          <RotateCcw size={9} />
          REPLAY SEQ
        </button>
      ...
      <div className="flex-1 w-full h-full relative cursor-crosshair">
        ...
        {/* Central Pupil warning HTML alert panel (blinks when clicked) */}
        <AnimatePresence>
          {systemAlert && (
            <motion.div 
            ...
            >
              <div className="relative w-64 h-64 rounded-full border border-orange-200 bg-white/95 backdrop-blur-md shadow-xl flex flex-col items-center justify-center p-6 text-center overflow-hidden">
                {/* Rotating accent rings */}
                <div className="absolute inset-2 rounded-full border border-dashed border-orange-300/40 animate-[spin_20s_linear_infinite]" />
                <div className="absolute inset-4 rounded-full border border-double border-orange-200/40 animate-[spin_15s_linear_infinite_reverse]" />
                
                {/* Threat Details */}
                <div className="z-10 flex flex-col items-center gap-1.5 w-full">
                  <ShieldAlert className="text-orange-600 animate-pulse" size={20} />
                  <span className="text-[9px] font-black text-orange-600 tracking-[0.2em] uppercase">
                    HAZARD DETECTED
                  </span>
                  
                  <div className="w-full text-left font-mono text-[7px] leading-tight text-slate-700 space-y-1 my-1 border-y border-slate-100 py-2">
                    <div className="text-orange-600 font-black animate-pulse text-center mb-1">
                      [ THREAT VECTOR IDENTIFIED ]
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-400">VECTOR INDEX:</span>
                      <span className="text-slate-800 font-bold">MUTATION_GNN_03</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-400">TARGET NODE:</span>
                      <span className="text-orange-600 font-bold">JAM_HONEY_TOKEN_03</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-400">EXFIL_PORT:</span>
                      <span className="text-slate-700 font-bold">8080 // OVERLAY</span>
                    </div>
                  </div>
                  ...
                  <div className="w-full space-y-1">
                    <div className="flex justify-between text-[6.5px] text-slate-400 font-mono">
                      <span>ISOLATING:</span>
                      <span className="text-orange-600 font-bold">{scanProgress}%</span>
                    </div>
                    <div className="w-full bg-slate-100 h-1 rounded-full overflow-hidden border border-slate-200">
                      <div 
                        className="bg-orange-600 h-full transition-all duration-75 shadow-sm"
                  ...
                  <button
                    ...
                    className="pointer-events-auto mt-2 bg-slate-100 hover:bg-slate-200 active:bg-slate-300 border border-slate-200 text-slate-700 font-black text-[7px] px-2 py-1 rounded tracking-widest uppercase transition-all cursor-pointer"
                  >
                    DISMISS
                  </button>
                </div>
        ...
        <div className="absolute bottom-5 left-1/2 -translate-x-1/2 pointer-events-none z-20 text-[8.5px] text-slate-500 tracking-widest text-center w-full max-w-[90%] px-4">
          {currentPhase === 'DESCENT' && (
            <span className="animate-pulse">PHASE 1: DESCENT SEQUENCE IN PROGRESS...</span>
          )}
          {currentPhase === 'SPLAY' && (
            <span className="text-orange-600 font-bold animate-pulse">PHASE 2: ASSEMBLING NEURAL IRIS PATTERN...</span>
          )}
          {currentPhase === 'IRIS' && (
            <div className="flex flex-col gap-1 items-center">
              <span className="text-slate-700 font-bold flex items-center gap-1.5 uppercase tracking-[0.25em] text-[8px] sm:text-[9px]">
                <Eye size={10} className="text-orange-500 animate-pulse shrink-0" /> Gaze Locked & Active. Gaze rotates based on cursor position.
              </span>
      ...
      <div className="px-6 py-4 border-t border-slate-200 bg-slate-50/80 backdrop-blur-sm z-20 flex justify-between items-center text-[8.5px] tracking-[0.2em] text-slate-500">
        <div className="flex items-center gap-1.5">
          <Sparkles className="text-orange-500" size={10} />
          <span>STABILITY: <span className="text-slate-800 font-bold">99.98%</span></span>
        </div>
        <div className="hidden sm:flex items-center gap-4">
          <span>COEFFICIENT: <span className="text-slate-800 font-bold font-mono">0.023b</span></span>
          <span className="w-[1px] h-3 bg-slate-200" />
          <span>FIBER NOISE: <span className="text-slate-800 font-bold">-112DB</span></span>
          <span className="w-[1px] h-3 bg-slate-200" />
          <span>DECEPTION LEVEL: <span className="text-orange-600 font-bold">OPTIMAL</span></span>
        </div>
        <div className="flex items-center gap-1">
          <span className="w-1.5 h-1.5 rounded-full bg-emerald-500" />
          <span className="text-emerald-600 font-bold font-mono uppercase">FEED_SYNCED</span>
        </div>
      </div>
    </div>
  );
